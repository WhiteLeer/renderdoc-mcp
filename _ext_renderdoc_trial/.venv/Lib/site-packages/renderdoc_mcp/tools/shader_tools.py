"""Shader analysis tools: disassemble_shader, get_shader_reflection, get_cbuffer_contents, save_gl_shader."""
...
def register(mcp: FastMCP):
    ...
    def save_gl_shader(
        ...
        """Save OpenGL/OpenGL ES shader source code to a text file.

        Extracts the shader source code from rawBytes and saves it as a .txt file.
        Works best with OpenGL/OpenGL ES captures where GLSL source is preserved.

        Args:
            stage: Shader stage (vertex, hull, domain, geometry, pixel, compute).
            output_path: Output file path for the shader source (e.g., "shader.txt").
            event_id: Optional event ID to navigate to first.
        """
        session = get_session()
        err = session.require_open()
        ...
        if refl.encoding == rd.ShaderEncoding.GLSL:
            encoding_name = "GLSL"
        elif refl.encoding == rd.ShaderEncoding.SPIRV:
            encoding_name = "SPIRV"
        elif refl.encoding == rd.ShaderEncoding.HLSL:
            encoding_name = "HLSL"
        elif refl.encoding == rd.ShaderEncoding.DXBC:
            encoding_name = "DXBC"

        # Try to decode as text
        try:
            shader_source = bytes(refl.rawBytes).decode('utf-8')
        except UnicodeDecodeError:
            return to_json(make_error(
                f"Shader encoding ({encoding_name}) is not text-based. Use disassemble_shader instead.",
                "API_ERROR"
            ))
        ...
        if output_dir and not os.path.exists(output_dir):
            try:
                ...
                return to_json(make_error(f"Failed to create output directory: {e}", "API_ERROR"))
        ...
        try:
            ...
            return to_json(make_error(f"Failed to write file: {e}", "API_ERROR"))
        ...
        return to_json({
            ...
            "size_bytes": len(refl.rawBytes),
            "line_count": len(shader_source.splitlines()),
        })
